...
- v3.411: fix AMOLED — клик по кнопке AMOLED после auto-update/reload не
  ...
  строки), хотя сам скринсейвер и статус работали. Причина: _amoledOn
  восстанавливается из localStorage как true сразу при загрузке, а
  настоящий браузерный fullscreen после reload сброшен; клик по кнопке
  AMOLED в этом состоянии срабатывал как toggle (true→false) и выключал
  режим вместо входа в fullscreen, общий "ловец первого тапа" на document
  не успевал среагировать (видел уже _amoledOn=false). Теперь toggleAmoled()
  сначала проверяет: если AMOLED включён, а document.fullscreenElement
  пуст — кнопка просто чинит fullscreen (_requestFS) без выключения режима;
  обычный вкл/выкл по кнопке работает как раньше.
====
...
- v3.361: fitness — режим насыщения по депозиту: при equity > 8000 плавно (до equity=12000)
  ...
  profit_weight×(1-mode×0.7) т.е. 4.0→1.2. При equity<=8000 поведение не меняется.
...
- v3.352: AMOLED-сейвер — полный откат к "богатой" версии редизайна 3.347
  ...
  мини-графиком свечей от точки входа, иконка статуса сети (Win10-style).
  Адаптировано под актуальные структуры данных: CANDLES {t,o,h,l,c} вместо
  массивов, SIGNALS s.dir/s.bar_i/s.open_end вместо старого формата,
  d.gate_balance вместо d.gate_bal. Заменяет v3.351 (упрощённый откат к
  ротации equity/цикл-время).
- v3.351: AMOLED-сейвер — откат экрана к виду версии 3.347 (ротация панелей
  equity / цикл+время каждые 30с, шрифт времени 4.4rem, подпись as-sub),
  отменены изменения v3.348-v3.350 (строка значений v1-v4, увеличенный шрифт
  5.8rem, статичный _amoledRender без ротации).
...
- v3.347: fix "график всё равно иногда перекрывает" — в _carry_forward_open_signal
  ...
  prev_open (старый "подвешенный" сигнал из предыдущего тика, exit_bar=None,
  без open_end) всё равно переносился и добавлялся в конец списка — получались
  ДВА "открытых" сигнала: новый genuinely-активный (с TP/SL+таймером,
  activeSig) и старый перенесённый (с заливкой/TP-SL до правого края от своего,
  более раннего, bar_i). Их зоны заливки визуально накладывались друг на друга.
  Теперь, если в new_signals уже есть open_end=True, carry-forward старого
  сигнала пропускается — "слот текущей открытой сделки" уже занят более новым.
...
- v3.338: фикс регрессии v3.337 — ограничение заливки правым краем следующего
  ...
  сделки). Теперь обрезка применяется только к закрытым сделкам (exit_bar!=null)
  и только если следующий сигнал реально входит на/до exit_bar текущей.
...
- v3.325: fix repaint HTF-фильтра — уже показанный на графике сигнал мог пропасть
  ...
  Причины в _build_htf_index:
  (1) Gate.io отдаёт текущую НЕЗАКРЫТУЮ HTF-свечу последним элементом — её close
      меняется каждую секунду, из-за чего EMA/direction последней точки индекса
      пересчитывался по-другому при каждом вызове → direction исторических баров
      рядом с этой точкой мог "переписаться" задним числом. Теперь незакрытая
      HTF-свеча отбрасывается перед расчётом.
  (2) EMA считалась нарастающим итогом начиная с candles[0], а стартовая свеча
      окна загрузки могла слегка отличаться между вызовами (из-за пагинации) →
      на участке "разогрева" EMA (ema_per точек) direction был чувствителен к
      стартовой точке и мог repaint'иться. Теперь warmup-участок (ema_per точек)
      отбрасывается из результата — отдаются только точки со сошедшейся EMA,
      не зависящие от стартовой свечи окна.
  Бары раньше первой точки индекса теперь получают htf_direction=0 (нейтрально,
  без ограничения long/short) — безопасный фоллбэк, не влияющий на repaint.
- v3.324: порог стагнации до встряски снижен 15→5 циклов — для уже "зрелого"
  конфига (загруженного из seed) 15 циклов означало слишком долгое ожидание
  первой встряски при многоминутных циклах. С эскалацией shake=2 (v3.323)
  более частые встряски не страшны — повторная встряска без улучшения сразу
  расширяет рескрамбл на числовые параметры фильтров.
...
- v3.312: дефолтный масштаб графика возвращён к 120/240 (мобайл/десктоп) — по
  скриншоту v3.311 (480/960) показывал слишком широкий диапазон, нужная область
  ~1/4 от него
...
opt_state = {
    ...
    "sw_restart_args": None,  # (symbol, tf, n_sw, alert_cfg, risk_pct, htf_index) — для watchdog-рестарта
    # live signal alert
                ...
                if (stale and not running or stale_watchdog) and best and (now - _last_refresh) > 60:
                    if stale_watchdog and running:
                        print(f"{_ts()} [SW] ⚠ Watchdog: данные протухли при running=True (last={last_t}, now={now}, "
                              f"отставание={now - last_t}с) — похоже SW-тред умер, перегружаю историю и перезапускаю поток...", flush=True)
                        with opt_lock:
                            restart_args = opt_state.get("sw_restart_args")
                        if restart_args:
                            r_symbol, r_tf, r_n_sw, r_alert_cfg, r_risk_pct, r_htf_index = restart_args
                            if r_symbol == symbol and r_tf == tf:
                                with opt_lock:
                                    opt_state["sw_running"] = False  # форсируем флаг — старый тред уже мёртв
                                new_sw_thread = threading.Thread(
                                    target=_sliding_window_thread,
                                    args=(r_symbol, r_tf, r_n_sw, r_alert_cfg, r_risk_pct),
                                    kwargs={"htf_index": r_htf_index},
                                    daemon=True
                                )
                                new_sw_thread.start()
                                print(f"{_ts()} [SW] ✅ Watchdog: SW-тред перезапущен для {r_symbol}/{r_tf}", flush=True)
                            else:
                                print(f"{_ts()} [SW] ⚠ Watchdog: символ/ТФ сменился с момента старта SW — рестарт пропущен", flush=True)
                        else:
                            print(f"{_ts()} [SW] ⚠ Watchdog: нет сохранённых restart_args — рестарт невозможен", flush=True)
                    else:
...
const _defaultViewLen=_isMob?30:60;
...
def _carry_forward_open_signal(prev_signals, new_signals, new_candles, prev_best_p=None, cur_best_p=None):
    ...
    if any(s.get("exit_bar") is None and not s.get("open_end") for s in new_signals):
        return new_signals
    # Если в new_signals уже есть сигнал с open_end=True (т.е. _simulate на новых
    # свечах сам видит активную позицию на конец данных) — "слот текущей открытой
    # сделки" уже занят этим (более новым) сигналом. prev_open по определению
    # старше по времени входа (стратегия не держит две позиции одновременно), и
    # его перенос привёл бы к ДВУМ "открытым" зонам на графике: заливка/TP-SL
    # перенесённого prev_open (до правого края) визуально НАКЛАДЫВАЛАСЬ на зону
    # genuinely-активного сигнала ("перекрывает") (v3.347).
    if any(s.get("open_end") for s in new_signals):
        return new_signals
...
def _build_htf_index(symbol, base_tf, days):
    ...
    try:
        ...
        # Отбрасываем последнюю свечу, если она ещё не закрылась (repaint-фикс):
        # Gate.io включает текущую формирующуюся HTF-свечу последним элементом —
        # её close меняется на каждый вызов, из-за чего EMA/direction последней
        # точки индекса "переписывался" задним числом при каждом пересчёте
        # (рестарт/новая свеча), и уже показанный на графике сигнал, который
        # зависел от этой точки, мог пропасть после рестарта несмотря на тот же конфиг.
        if candles and (candles[-1]["t"] + interval_sec) > int(time.time()):
            candles = candles[:-1]
        if len(candles) < 50:
            return None
        ...
        print(f"[HTF] index построен: {htf_tf} {symbol} — {len(result)} точек (warmup {ema_per} отброшен), "
        ...
        # Отбрасываем разогревочный участок EMA — на этом отрезке EMA ещё не сошлась,
        # и её значение (а значит direction) заметно зависит от того, какая свеча
        # оказалась первой в загруженном окне. При следующем построении (рестарт/
        # новая свеча) окно может начаться чуть раньше/позже → разные EMA на этом
        # участке → repaint direction для ранних точек индекса. Точки после warmup
        # практически не зависят от стартовой свечи (EMA уже сошлась).
        result = result[ema_per:]
        if len(result) < 20:
            return None
        return result
    except Exception as e:
...
def _permutation_test(base_params, candles, days, risk_pct, n_iter=2000):
    ...
    Делает два независимых теста на одном и том же наборе реальных PnL сделок
    (без повторного прогона стратегии — дёшево, поэтому n_iter может быть большим):

    1) Sign-scramble significance test (главный):
       Нулевая гипотеза: знак каждой сделки — это монетка 50/50 (т.е. размер хода
       определяется параметрами риска как обычно, но НЕТ направленного edge).
       Берём |pnl| каждой сделки и n_iter раз случайно переподбрасываем знак.
       Если итоговая доходность реальной последовательности выше, чем почти все
       n_iter случайных вариантов — это статистически значимый edge, а не везение.
       p_value = доля случайных вариантов с доходностью >= реальной.

    2) Trade-order permutation (путевой риск):
       Тот же набор сделок (с реальными знаками), но в случайном порядке —
       сумма (итоговая доходность) не меняется, а вот max_dd меняется сильно.
       Показывает, насколько "повезло" реальной последовательности с порядком
       (могла ли просадка быть гораздо хуже при другом порядке тех же сделок).
    """
    r = _simulate(candles, base_params, days, risk_pct=risk_pct)
    if not r or r.get("trades", 0) < 15:
        return {"ok": False, "error": "слишком мало сделок для статистики (нужно ≥15)"}
    ...
    # ── (1) sign-scramble: нулевая гипотеза "edge отсутствует" ──
    null_totals = []
    ge_count = 0
    for _ in range(n_iter):
        tot = 0.0
    ...
    # ── (2) trade-order permutation: путевой риск (max_dd) ──
    def _max_dd_for_order(seq):
        eq = 100.0
        peak = 100.0
        dd_worst = 0.0
        for x in seq:
    ...
    actual_dd = r.get("max_dd", _max_dd_for_order(pnls))
    shuffled = list(pnls)
    dd_samples = []
    worse_count = 0
    for _ in range(n_iter):
        random.shuffle(shuffled)
        dd = _max_dd_for_order(shuffled)
        dd_samples.append(dd)
        if dd >= actual_dd:
            worse_count += 1
    dd_samples.sort()
    dd_percentile = 100.0 * sum(1 for x in dd_samples if x <= actual_dd) / n_iter

    return {
        "ok": True,
        "trades": n,
        "actual_total_return": round(actual_total, 2),
        "actual_winrate": r.get("winrate"),
        "actual_max_dd": round(actual_dd, 2),
        "significance": {
            "p_value": round(p_value, 4),
            "null_mean_return": round(null_mean, 2),
            "null_std_return": round(null_std, 2),
            "verdict": ("значимо: p<0.05 — edge вероятно реальный" if p_value < 0.05
                        else "НЕ значимо: p>=0.05 — неотличимо от случайного 50/50 знака сделок, "
                             "похоже на переобучение/везение"),
        },
        "path_risk": {
            "dd_percentile": round(dd_percentile, 1),
            "dd_median_random_order": round(dd_samples[n_iter // 2], 2),
            "dd_worst_random_order": round(dd_samples[-1], 2),
            "note": ("текущая просадка близка к лучшему возможному случаю — реальная "
                     "просадка в будущем может оказаться заметно выше" if dd_percentile < 50
                     else "текущая просадка типична или хуже типичной для данного набора сделок"),
        },
    }
...
def run_optimizer(params):
    ...
    _STAGNATION_THRESH  = 5   # порог: столько циклов без улучшения → встряска
    ...
    while True:
        ...
        if final_result:
            ...
            if not sw_already:
                with opt_lock:
                    opt_state["sw_restart_args"] = (symbol, tf, n_sw, alert_cfg, risk_pct, _htf_index)
...
function _amoledMiniChart(candles, entryIdx, curPrice, isLong){
  ...
  C.forEach((c,i)=>{
    ...
    bars+=`<line x1="${mx}" y1="${py(c.h)}" x2="${mx}" y2="${y1}" stroke="${col}" stroke-width="1"/>`;
...
function toggleAmoled(){
  // v3.411: fix — после auto-update/reload _amoledOn восстанавливается из
  // localStorage как true, но реальный браузерный Fullscreen сбрасывается
  // (requestFullscreen() нельзя вызвать программно при загрузке без жеста
  // пользователя). Раньше клик по кнопке AMOLED в этом состоянии сразу же
  // ВЫКЛЮЧАЛ режим (_amoledOn уже true → toggle делал false → _exitFS()),
  // и общий "ловец первого тапа" на document не успевал сработать, т.к.
  // _amoledOn к моменту всплытия клика был уже сброшен в false. Снаружи это
  // выглядело так: жмёшь AMOLED — ничего не разворачивается, кусок браузера
  // (адресная строка/навигация) остаётся виден. Теперь: если AMOLED уже
  // включён, а реального fullscreen нет — кнопка чинит fullscreen вместо
  // выключения режима; полноценный toggle (вкл/выкл) работает как раньше.
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        elif parsed.path == "/permutation_test":
            with opt_lock:
                tb = opt_state.get("trade_best") or opt_state.get("all_time_best")
                days = opt_state.get("days", 30)
            if not tb or not tb.get("params"):
